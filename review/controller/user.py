...
def register():
    ...
    if request.method == "POST":
        ...
        user = review.model.user.get_by_email(request.form["email"], filter=False)
...
def profile(username):
    ...
    return review.renderer.render('Hi {{person}}', {'person': 'test'})

    user = review.model.user.get_by_username(username)
    uploads = review.model.upload.Upload.get_where({'user_id': user.id})
    comments = review.model.comment.Comment.get_where({'user_id': user.id})
    files = review.model.file.File.get([e.file_id for e in comments.values()])

    upload_count = review.model.upload.Upload.get_count_where({'user_id': user.id})
    comment_count = review.model.comment.Comment.get_count_where({'user_id': user.id})
